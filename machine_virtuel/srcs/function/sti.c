...
/*   Updated: 2016/12/20 16:39:57 by vlancien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
** sti : Opcode 11. Prend un registre, et deux index
** (potentiellement des registres).
** Additionne les deux derniers, utilise cette somme
** comme une adresse ou sera copiée la valeur du premier paramètre.
...
int		get_i02_func_sti(t_type_func list, t_env *e, int xproc, int *place)
{
	...
	{
		i = (short)to_int_getx(get_x(e->process[xproc].position +
			*place - 2, e->process[xproc].position + *place)) % IDX_MOD;
		i = (short)to_int_getx(get_x(e->process[xproc].position + i,
			e->process[xproc].position + i + 2));
	}
	else if (list.type[1].t_dir && (*place += 2))
		i = (short)to_int_getx(get_x(e->process[xproc].position + *place - 2,
...
void	sti_func(t_env *e, int xproc, t_type_func list)
{
	...
	place = 3;
	i[0] = to_int_getx(get_x(e->process[xproc].position + 2,
	...
	{
		write_from_x(e, (e->process[xproc].position + i[3]),
		e->process[xproc].reg[i[0]], 4);
		write_from_tab2((e->process[xproc].position + i[3]),
		4, e->process[xproc].id_player + 1);
	}
