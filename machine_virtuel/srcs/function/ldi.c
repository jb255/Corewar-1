...
/*   Updated: 2016/12/19 21:36:37 by vlancien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_ldi_arg(t_type_a list, t_env *e, int xproc, int *place)
{
	...
	else if (list.t_ind && (*place += 2))
	{
		...
		i = to_int_getx(get_x(e, e->process[xproc].position + i, e->process[xproc].position + i + 4)) % IDX_MOD;
	}
...
void	ldi_func(t_env *e, int xproc, t_type_func list)
{
	...
	i[3] = to_int_getx(get_x(e, e->process[xproc].position + place, e->process[xproc].position + (place + 1))); // Arg3
	// ft_printf_fd(e->fd, " r%d\n", i[3]);
	i[2] = ((i[1] + i[0])); // Ajout de l'arg 2 a l'arg 1
	// ft_printf_fd(e->fd, "load from %d + %d = %d (with pc and mod %d)\n",i[1], i[0], i[2], e->process[xproc].position + i[2] );
