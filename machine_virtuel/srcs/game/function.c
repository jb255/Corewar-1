...
/*   Updated: 2016/12/16 02:34:54 by vlancien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_type_func		op_size_analys(t_type_func list, int x, int index, char *op_size)
{
	if (op_size[index] == '0' && op_size[index + 1] == '1' && (list.size += 1))
		list.type[x].t_reg++;
	else if (op_size[index] == '1' && op_size[index + 1] == '0' && (list.size += 4))
	{
		list.type[x].t_dir++;
		if (list.func == 10 || list.func == 11 || list.func == 13)
			list.size -= 2;
	}
	else if (op_size[index] == '1' && op_size[index + 1] == '1' && (list.size += 2))
		list.type[x].t_ind++;
	return (list);
}
